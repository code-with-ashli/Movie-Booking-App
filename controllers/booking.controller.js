const { bookingCreationValidationSchema, verifyPaymentValidationSchema } = require('../lib/validators/booking.validator')
...
async function handleCreateBookings(req, res){
    const validationStatus  = await bookingCreationValidationSchema.safeParseAsync(req.body)

    if (validationStatus.error)
        return res.status(400).json({ error: validationStatus.error })

    const { seatNumber, showId} = validationStatus.data
    console.log(showId);
    ...
    if(alreadyBooked)  return res.status(400).json({error: 'Already Booked'})
...
module.exports = {handleCreateBookings, verifyPayment }